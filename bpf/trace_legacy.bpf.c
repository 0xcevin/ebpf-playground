...
/* Temporary storage for write/read/sendto/recvfrom enter fd keyed by pid */
struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__uint(max_entries, 1024);
	__type(key, u32);
...
{
	u16 family;
	if (bpf_probe_read(&family, sizeof(family), &addr->sa_family))
		return;
	info->family = family;
	if (family == AF_INET) {
		struct sockaddr_in sin;
		if (bpf_probe_read(&sin, sizeof(sin), addr) == 0) {
			info->daddr = sin.sin_addr.s_addr;
			info->dport = sin.sin_port;
		}
	} else if (family == AF_INET6) {
		struct sockaddr_in6 sin6;
		if (bpf_probe_read(&sin6, sizeof(sin6), addr) == 0) {
...
static __always_inline void check_and_alert(void *ctx, struct conn_key *key, struct conn_info *info, int is_tx)
{
	u32 cfg_key = 0;
...
int tracepoint__sys_exit_write(struct trace_event_raw_sys_exit *ctx)
{
	...
	info->tx_bytes += size;
	check_and_alert(ctx, &key, info, 1);
...
int tracepoint__sys_exit_read(struct trace_event_raw_sys_exit *ctx)
{
	...
	info->rx_bytes += size;
	check_and_alert(ctx, &key, info, 0);
